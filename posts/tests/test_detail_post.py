...
class TestPostDetailView(TestCase):
    def setUp(self) -> None:
        self.user = User.objects.create(phone="1234567890", password="123qwe456ert")
        self.post_1 = PostModel.objects.create(
            title="Detail Post", post_owner=self.user, is_free=True
        )
        self.post_2 = PostModel.objects.create(
            title="Detail_2 Post_2", post_owner=self.user, is_free=True
        )
